globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/worker-entry-aNBLm7-1.js"];
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "governdiff-reviewer-0.1.0";
	}
};
...
var _cacheFallbackState = _g$3[_FALLBACK_KEY$2] ??= {
	requestScopedCacheLife: null,
	unstableCacheRevalidation: "foreground"
};
...
function buildAppPageElements(options) {
	...
	const orderedTreePositions = Array.from(/* @__PURE__ */ new Set([...layoutEntries.map((entry) => entry.treePosition), ...templateEntries.map((entry) => entry.treePosition)])).sort((left, right) => left - right);
...
var reviewer_workspace_default = /* #__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "cc78faf55494", "default");
//#endregion
//#region app/page.tsx
...
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(reviewer_workspace_default, {});
}
...
function __isrCacheKey(pathname, suffix) {
	...
	const key = "app:governdiff-reviewer-0.1.0:" + normalized + ":" + suffix;
	if (key.length <= 200) return key;
	return "app:governdiff-reviewer-0.1.0:__hash:" + __isrFnv1a64(normalized) + ":" + suffix;
}
...
async function _handleRequest(request, __reqCtx, _mwCtx) {
	...
	const progressiveActionResponse = await handleProgressiveServerActionRequest({
		actionId,
	...
	if (request.method === "POST" && actionId) {
		...
		try {
			let body;
			try {
				body = actionContentType.startsWith("multipart/form-data") ? await __readFormDataWithLimit(request, __MAX_ACTION_BODY_SIZE) : await __readBodyWithLimit(request, __MAX_ACTION_BODY_SIZE);
			} catch (sizeErr) {
	...
	if (!match) {
		const notFoundResponse = await renderNotFoundPage(null, isRscRequest, request, void 0, _scriptNonce, _mwCtx);
		if (notFoundResponse) return notFoundResponse;
		setHeadersContext(null);
		setNavigationContext(null);
		const notFoundHeaders = new Headers();
		mergeMiddlewareResponseHeaders(notFoundHeaders, _mwCtx.headers);
		...
		renderSpecialError(__buildSpecialError) {
			return buildAppPageSpecialErrorResponse({
				...
				renderFallbackPage(statusCode) {
					return renderHTTPAccessFallbackPage(route, statusCode, isRscRequest, request, { matchedParams: params }, _scriptNonce, null);
				},
				requestUrl: request.url,
